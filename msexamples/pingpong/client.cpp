...
class PingSession : boost::noncopyable
{
public:
    PingSession(EventLoop* loop,
        ...
        messagesRead_(0)
    {
        client_.setConnectionCallback(
            boost::bind(&PingSession::onConnection, this, _1));
        client_.setMessageCallback(
            boost::bind(&PingSession::onMessage, this, _1, _2, _3));
    }
...
public:
        ...
        timeout_(timeout)
    {
        ...
        for (int i = 0; i < sessionCount; ++i)
        {
            ...
            PingSession* session = new PingSession(threadPool_.getNextLoop(), serverAddr, buf, this);
            session->start();
...
private:
    ...
    int sessionCount_;
    int timeout_;
    std::vector<std::unique_ptr<PingSession>> sessions_;
    string message_;
    AtomicInt32 numConnected_;
};

void PingSession::onConnection(const TcpConnectionPtr& conn)
{
    if (conn->connected())
    {
