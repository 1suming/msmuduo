...
{
	ms::LogStream os;
	const ms::LogStream::Buffer& buf = os.buffer();
...
BOOST_AUTO_TEST_CASE(testLogStreamIntegerLimits)
{
	...
	os << (std::numeric_limits<int16_t>::min)(); //由于windows.h也定义了min，max宏，会冲突，所以把min前后用小括号括起来，在满足C++语法的同时，骗过了编译器的宏替换：
...
BOOST_AUTO_TEST_CASE(testLogStreamFmts)
{
	...
	os << ms::Fmt("%4.2f", 1.2) << ms::Fmt("%4d", 43);
