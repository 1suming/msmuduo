...
public:
	...
	/*
	retrieve return void,to prevent string str(retrieve(readablebytes()),readableBytes)(),
	the evaluation of two function are unspecifed
	*/
	...
	void shrink(size_t reserve)
	{
		...
		other.append(toStringPiece());//复制内容
	...
	void makeSpace(size_t len)
	{
		...
		else
		{
			...
			std::copy(begin() + readerIndex_, begin() + writerIndex_,
				begin() + kCheapPrepend);
			readerIndex_ = kCheapPrepend;
			writerIndex_ = readerIndex_ + readable; //这里我之前出现了错误，写成了readerIndex+ readableBytes(),导致后面的assert失败，看了assert是个好东西

			assert(readable == readableBytes());//之前的readable bytes应该相等
